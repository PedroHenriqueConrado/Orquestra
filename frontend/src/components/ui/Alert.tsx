...
const Alert: React.FC<AlertProps> = ({ type, message, onClose }) => {
  ...
  const alertStyles = {
    success: {
      bg: theme === 'dark' ? 'bg-green-900' : 'bg-green-50',
    ...
    error: {
      bg: theme === 'dark' ? 'bg-red-900' : 'bg-red-50',
    ...
    warning: {
      bg: theme === 'dark' ? 'bg-yellow-900' : 'bg-yellow-50',
    ...
    info: {
      ...
      text: theme === 'dark' ? 'text-blue-200' : 'text-blue-700',
      icon: (
          ...
          <div className="ml-auto pl-3">
            <div className="-mx-1.5 -my-1.5">
              <button
                type="button"
                onClick={onClose}
                className={`inline-flex rounded-md p-1.5 ${style.bg} ${style.text} hover:bg-opacity-80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-${style.bg} focus:ring-${style.text}`}
              >
